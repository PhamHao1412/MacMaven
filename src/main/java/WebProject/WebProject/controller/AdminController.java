...
//			model.addAttribute("recentCustomer", recentCustomer);
	...
	public String DashboardWalletView(Model model) {
		...
		if (admin == null) {
			...
			List<Order> listOrder = orderService.findAll();
			List<Order> listPaymentWithMomo = orderService.findAllByPayment_Method("Pay with Momo");
	...
	public String DeleteProduct(@PathVariable int id, Model model, HttpServletRequest request) throws Exception {
		...
		if (admin == null) {
			...
			if (product != null) {
				for (ProductImage y : product.getProductImage()) {
					productImageService.deleteById(y.getId());
				}
...
//			int category_selected = 0;
	...
	@GetMapping("/dashboard-orderList/{status}")
	public String orderList(@PathVariable("status") String status, Model model) {
		List<Order> orderList = orderService.getOrderByStatus(status);
		String titleOrder = status;
		model.addAttribute("orderList", orderList);
		model.addAttribute("titleOrder", titleOrder);
		return "dashboard-orderList";
	}
	@GetMapping("/dashboard-orderDetail/{id}")
	public String orderDetail(@PathVariable("id") int id, Model model) {
		Order infOrder = orderService.findById(id);
		List<Order_Item> infoOrderDetail = order_ItemService.getAllByOrder_Id(id);
		model.addAttribute("infoOrder", infOrder);
		model.addAttribute("infoOrderDetail", infoOrderDetail);
		return "dashboard-orderDetail";
	}
	@PostMapping("/dashboard-updateStatus")
...
//			String customerEmail = user.getEmail();
			...
			String body = "Cảm ơn bạn đã đặt hàng! Đơn hàng của bạn đã được xác nhận.";
