...
public class ProductControler {
	...
	public String listStudents(Model model) throws Exception {
		Cookie token = cookie.read("JWT");
		Cookie tokenGoogle = cookie.read("IdToken");
		...
		if (remember != null && token !=null) {
			String jwt = token.getValue();
				if (tokenProvider.validateToken(jwt)) {
					String user_id = tokenProvider.getUserIdFromJWT(jwt);
					 acc = userService.findByIdAndRole(user_id, "user");
				}
			}
		else if (remember != null && tokenGoogle !=null) {
			String idToken = tokenGoogle.getValue();
			acc = userService.findByIdAndRole(idToken, "user");
		}
		if(acc != null){
